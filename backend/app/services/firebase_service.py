...
class FirestoreUserService:
    ...
    def create_goal(self, user_id: str, goal_data: dict):
        """Create a new goal for a user"""
        try:
            goal_ref = db.collection('users').document(user_id).collection('goals').document(goal_data['id'])
            goal_ref.set(goal_data)
            return True
        except Exception as e:
            print(f"Failed to create goal for user {user_id}: {e}")
            return False

    def get_user_goals(self, user_id: str, status_filter: str = None):
        """Get all goals for a user, optionally filtered by status"""
        try:
            goals_ref = db.collection('users').document(user_id).collection('goals')
            
            if status_filter:
                goals = goals_ref.where('status', '==', status_filter).stream()
            else:
            ...
            goal_list.sort(key=lambda x: x.get('createdDate', ''), reverse=True)
            return goal_list
        except Exception as e:
            print(f"Failed to get goals for user {user_id}: {e}")
            return []

    def update_goal(self, user_id: str, goal_id: str, updates: dict):
        """Update a goal"""
        try:
            ...
            print(f"Failed to update goal {goal_id} for user {user_id}: {e}")
            return False
    ...
    def delete_goal(self, user_id: str, goal_id: str):
        ...
        try:
            ...
            print(f"Failed to delete goal {goal_id} for user {user_id}: {e}")
            return False

    def get_goal_stats(self, user_id: str):
        ...
        try:
            ...
            print(f"Failed to get goal stats for user {user_id}: {e}")
            return {
                "total_goals": 0,
                "active_goals": 0,
                "completed_goals": 0,
